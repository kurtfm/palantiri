...
module.exports = (report) => {
    return new Promise((resolve, reject) => {
        ...
        if ('object' !== typeof folders) {
            reject({error: 'Collection should be broken into folders in order for processing and formatting to work'});
        }
            ...
            for (var o = 0; o < order.length; o++) {
                ...
                for (var r = 0; r < results.length; r++) {
                    if (report.results[r].id === order[o]) {
                        ...
                        for (var t = 0; t <  report.collection.debugInfo.length; t++) {
                            if (report.collection.debugInfo[t].id === healthcheck.folders[f].tests[o].id) {
                                healthcheck.folders[f].tests[o].statusCode = report.collection.debugInfo[t].statusCode;
                                healthcheck.folders[f].tests[o].responseTime = report.collection.debugInfo[t].totalTime.replace(/ms/g, '');
                                break;
                            }
