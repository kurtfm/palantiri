...
var tests = require('../test/app/resources/newman/onetest-tests.json');
...
.on('test', function(err, args) {
    ...
    console.log('test tags: ',
      testConfig.tags,
      'test metric: ',
      typeof testConfig.metric !== 'undefined' ? testConfig.metric : '');
    console.log('response time: ',
      args.executions[0].result.globals.responseTime);
    for (var key in args.executions[0].result.globals.tests) {
      console.log('test result: ',
        args.executions[0].result.globals.tests[key] ? 1 : 0);
    }
    console.log('---------------------------------------------------');
  ...
  .once('done', function(err, summary) {
    console.info(
      `The collection run completed ${err ? 'with' : 'without'} error(s).`);
    ...
    console.log(util.inspect(fullDebugData, {
      depth: 15,
      colors: true
    }));
