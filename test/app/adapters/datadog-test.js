...
describe('Datadog Adapter Tests', function() {
    ...
    /** 
        it('should handle an error from hot-shots client',
            function*() {
                delete require.cache[require.resolve(conf.application_root + '/app/adapters/datadog.js')];
                var DatadogFailures = proxyquire(conf.application_root + '/app/adapters/datadog.js', {
                    'hot-shots': stubDatadogClientError
                });

                var datadog = new DatadogFailures(conf.metrics_prefix);
                return datadog.sendCount('mock', 1, ['bogus', 'tester'])
